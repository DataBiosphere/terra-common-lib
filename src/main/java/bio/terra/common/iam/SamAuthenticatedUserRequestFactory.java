...
/**
 * Used verify a user is enabled and to get accurate email and id for the user from Sam. Email and
 * id embedded in the request's authentication token are not always accurate (in the case of pet
 * identities) and should not be used. Please do not use by default on every request, instead use
 * {@link TokenAuthenticatedRequestFactory} Use this factory only when user id and email are
 * actually required or to verify a user is enabled. These cases should be in the minority because
 * every /api call to Sam checks if the user is enabled and most service api calls should check a
 * Sam permission. Emails should be seldom used within the system and should not be stored as they
 * may change.
 */
@Component
public class SamAuthenticatedUserRequestFactory {
  ...
  public SamUserAuthenticatedRequest from(
      HttpServletRequest servletRequest, ApiClient samApiClient) {
    return from(tokenAuthenticatedRequestFactory.from(servletRequest), samApiClient);
  }

  public SamUserAuthenticatedRequest from(
      TokenAuthenticatedRequest tokenAuthenticatedRequest, ApiClient samApiClient) {
    SamUserAuthenticatedRequest.Builder builder =
        SamUserAuthenticatedRequest.builder().setTokenRequest(tokenAuthenticatedRequest);
